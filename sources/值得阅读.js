...
//搜索
const search = (key) => {
  let response = GET(`https://s.fjwhgs.com/v1/lists.api?keyword=${encodeURI(key)}`,{headers:header})
  let array = []
...
//详情
const detail = (url) => {
  let response = GET(`https://book.fjwhgs.com/details/${url}.html`,{headers:header})
  let $ = JSON.parse(response).data
  let v = JSON.parse(GET(`https://book.fjwhgs.com/source/${url}.html`,{headers:header})).data[0]
  let book = {
    summary: $.remark,
